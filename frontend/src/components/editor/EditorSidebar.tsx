...
import { GripVertical, Trash2, Plus, ArrowUp, ArrowDown, Copy, ChevronDown, ChevronUp, ChevronsUp, ChevronsDown, Eye, EyeOff, Upload, Loader2, X } from "lucide-react";
...
function GalleryProperties({ config, onChange }: { config: any; onChange: (c: any) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Background toggle */}
      <div className="flex items-center justify-between py-2 px-3 bg-zinc-800 rounded-lg border border-zinc-700">
        <div>
          <p className="text-xs font-semibold text-zinc-300">Background</p>
          <p className="text-[10px] text-zinc-500 mt-0.5">{config.darkBackground ? "Dark (black)" : "Light (white)"}</p>
        </div>
        <button
          onClick={() => set("darkBackground", !config.darkBackground)}
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
            config.darkBackground ? "bg-zinc-600" : "bg-indigo-500"
          }`}
        >
          <span className={`inline-block h-3.5 w-3.5 transform rounded-full transition-transform ${
            config.darkBackground ? "translate-x-1 bg-zinc-300" : "translate-x-5 bg-white"
          }`} />
        </button>
      </div>
              ...
              sections.map((section, index) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex flex-col rounded-xl cursor-pointer transition-all border ${
                      isActive
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 px-2.5 py-2">
                      ...
                      {/* Inline quick-action icons on hover (non-active) */}
                      {!isActive && (
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => updateSection(section.id, { ...section.config, isHidden: !section.config?.isHidden })} className="p-1 hover:bg-zinc-700 rounded text-zinc-400 hover:text-white" title={section.config?.isHidden ? "Show" : "Hide"}>
                    ...
                    {/* Active section: always-visible action bar */}
                    {isActive && (
                      <div
                        className="px-2 pb-2 flex items-center gap-1"
                        onClick={(e) => e.stopPropagation()}
                      >
                        {/* Order controls */}
                        <div className="flex items-center bg-zinc-800 border border-zinc-700 rounded-lg overflow-hidden">
                          <button
                            onClick={() => reorderSections(index, 0)}
                            disabled={index === 0}
                            className="px-1.5 py-1 text-amber-400 hover:bg-zinc-700 disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
                            title="Bring to Front (top)"
                          ><ChevronsUp className="w-3.5 h-3.5" /></button>
                          <button
                            onClick={() => { if (index > 0) reorderSections(index, index - 1); }}
                            disabled={index === 0}
                            className="px-1.5 py-1 text-zinc-300 hover:bg-zinc-700 border-l border-zinc-700 disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
                            title="Move Up"
                          ><ChevronUp className="w-3.5 h-3.5" /></button>
                          <button
                            onClick={() => { if (index < sections.length - 1) reorderSections(index, index + 1); }}
                            disabled={index === sections.length - 1}
                            className="px-1.5 py-1 text-zinc-300 hover:bg-zinc-700 border-l border-zinc-700 disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
                            title="Move Down"
                          ><ChevronDown className="w-3.5 h-3.5" /></button>
                          <button
                            onClick={() => reorderSections(index, sections.length - 1)}
                            disabled={index === sections.length - 1}
                            className="px-1.5 py-1 text-amber-400 hover:bg-zinc-700 border-l border-zinc-700 disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
                        ...
                        <div className="flex items-center bg-zinc-800 border border-zinc-700 rounded-lg overflow-hidden">
                          <button
                            onClick={() => updateSection(section.id, { ...section.config, isHidden: !section.config?.isHidden })}
                            className="px-1.5 py-1 text-zinc-300 hover:bg-zinc-700 transition-colors"
                            title={section.config?.isHidden ? "Show Section" : "Hide Section"}
                          >
                            {section.config?.isHidden ? <EyeOff className="w-3.5 h-3.5 text-red-400" /> : <Eye className="w-3.5 h-3.5" />}
                          </button>
                          <button
                            onClick={() => duplicateSection(section.id)}
                            className="px-1.5 py-1 text-zinc-300 hover:text-indigo-300 hover:bg-zinc-700 border-l border-zinc-700 transition-colors"
                            title="Duplicate"
                          ><Copy className="w-3.5 h-3.5" /></button>
                          <button
                            onClick={() => removeSection(section.id)}
                            className="px-1.5 py-1 text-zinc-300 hover:text-red-400 hover:bg-zinc-700 border-l border-zinc-700 transition-colors"
                            title="Delete"
