  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(errData.detail || "Failed to create quotation.");
      }
    } catch (err) {
      console.error(err);
      alert("Network error");
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Create Quotation</h1>
          <p className="text-gray-400 mt-1">Build a custom quotation for your client</p>
        </div>
      </div>
