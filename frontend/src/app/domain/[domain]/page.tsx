...
async function getLandingPage(domain: string, visitorId?: string, lang?: string) {
  try {
    const url = new URL(`${API}/pages/domain/${domain}`);
...
export async function generateMetadata({ params }: { params: Promise<{ domain: string }> }) {
  ...
  const page = await getLandingPage(domain, visitorId, lang);
  ...
  return (
    <main className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className={page.logo_url ? "pt-16" : ""}>
        {page.sections?.map((section: any) => renderSection(section))}
      </div>
